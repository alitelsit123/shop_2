...
if(isset($_POST['update_product'])){
   ...
   mysqli_query($conn, "UPDATE `products` SET name = '$name', details = '$details', price = '$price', status = '$status' WHERE id = '$update_p_id'") or die('query failed');
...
<form action="" method="post" enctype="multipart/form-data">
   ...
   <select name="status" class="box" id="">
    <option value="available" <?= $fetch_products['status'] == 'available' ? 'selected':'' ?>>Tersedia</option>
    <option value="unavailable" <?= $fetch_products['status'] == 'unavailable' ? 'selected':'' ?>>Tidak Tersedia</option>
   </select>
...
<?php
      ...
      echo '<p class="kosong">tidak ada pembaruan produk pilih</p>';
   }
?>
